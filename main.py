...
**Click Bellow Button To Join Now.👇🏻**''', buttons=Button.url('❇️ ᴜᴘᴅᴀᴛᴇs ᴄʜᴀɴɴᴇʟ ❇️', f'https://t.me/{Config.UPDATES_CHANNEL_USERNAME}'))
        ...
        txt = await event.reply('** 🦋 Preparing results for "{}" 🔍**'.format(event.text))
...
**Check Spelling On** [Google](http://www.google.com/search?q={event.text.replace(' ', '%20')}%20Movie) 🔍
    '''
        ...
        message = f'**ᴄʟɪᴄᴋ ʜᴇʀᴇ 👇🏻 ꜰᴏʀ "{event.text}"**\n\n[♻️📽️ {str(event.text).upper()}\n📝🔎 {str("click here for result").upper()}]({tgraph_result})', buttons=Button.url('Join Updates Channel', 'https://t.me/Imdb_updates')]
